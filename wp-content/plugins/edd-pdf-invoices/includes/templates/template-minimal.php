<?php
/**
 * Minimal PDF Invoice Template
 *
 * Builds and renders the minimal PDF invoice template .
 *
 * @since 1.0
 *
 * @uses HTML2PDF
...
function eddpdfi_pdf_template_minimal( $eddpdfi_pdf, $eddpdfi_payment, $eddpdfi_payment_meta, $eddpdfi_buyer_info, $eddpdfi_payment_gateway, $eddpdfi_payment_method, $address_line_2_line_height, $company_name, $eddpdfi_payment_date, $eddpdfi_payment_status ) {
	global $edd_options;
	...
	$eddpdfi_pdf->SetXY( 20, 55 );

	$eddpdfi_pdf->SetFont( $fontb, 'B', 10 );
...
$eddpdfi_pdf->Cell( 0, 6, $country, 0, 2, 'L', false );
	...
	$eddpdfi_pdf->Rect( 20, 132, 170, 0.5, 'F' );
	...
	if ( $eddpdfi_pdf_downloads ) {
		...
		foreach ( $eddpdfi_pdf_downloads as $key => $cart_item ) {
			...
			if ( eddpdfi_item_quantities_enabled() ) {
				$eddpdfi_pdf->MultiCell( 102, $linecount * 4, $eddpdfi_download_title, 0, 'L', false, 0, 20 );
				...
				$eddpdfi_pdf->MultiCell( 122, $linecount * 4, $eddpdfi_download_title, 0, 'L', false, 0, 20 );
				$eddpdfi_pdf->Cell( 45, $linecount * 4, $eddpdfi_download_price, 0, 2, 'R', false );
			}
		}
		...
		$eddpdfi_pdf->SetX( 35 );
		$eddpdfi_pdf->Cell( 0, 8, __( 'Subtotal', 'eddpdfi' ) . ' - ' . $subtotal, 0, 2, 'R', false );

		if ( edd_use_taxes() ) {
			$eddpdfi_pdf->SetX( 35 );
			$eddpdfi_pdf->Cell( 0, 11, __( 'Tax', 'eddpdfi' ) . ' - ' . $tax, 0, 2, 'R', false );
		}
		...
		$eddpdfi_pdf->SetX( 35 );
		$eddpdfi_pdf->Cell( 0, 8, __( 'Discount Used', 'eddpdfi' ) . ' - ' . $eddpdfi_discount, 0, 2, 'R', false );
		$eddpdfi_pdf->Cell( 0, 11, __( 'Total Paid', 'eddpdfi' ) . ' - ' . $total, 0, 2, 'R', false );

		$eddpdfi_pdf->Ln( 10 );

		if ( isset ( $edd_options['eddpdfi_additional_notes'] ) && ! empty( $edd_options['eddpdfi_additional_notes'] ) ) {
			$eddpdfi_pdf->SetX( 35 );
			$eddpdfi_pdf->SetFont( $fontb, '', 13 );
			$eddpdfi_pdf->SetTextColor( 224, 65, 28 );
...
add_action( 'eddpdfi_pdf_template_minimal', 'eddpdfi_pdf_template_minimal', 10, 10 );