<?php
/**
 * Blue Stripe PDF Invoice Template
 *
 * Builds and renders the blue stripe PDF invoice template.
 *
 * @since 1.0
 *
 * @uses HTML2PDF
...
function eddpdfi_pdf_template_blue_stripe( $eddpdfi_pdf, $eddpdfi_payment, $eddpdfi_payment_meta, $eddpdfi_buyer_info, $eddpdfi_payment_gateway, $eddpdfi_payment_method, $address_line_2_line_height, $company_name, $eddpdfi_payment_date, $eddpdfi_payment_status ) {
	global $edd_options;
	...
	$eddpdfi_pdf->SetFont( 'droidserif', '', 12, '', true );

	$eddpdfi_pdf->AddPage();

	$eddpdfi_pdf->SetX( 35 );

	$eddpdfi_pdf->SetFont( 'droidserif', '', 12 );
	...
	$eddpdfi_pdf->SetXY( 150, 77 );
	$eddpdfi_pdf->SetFillColor( 149, 210, 236 );

	if ( isset( $edd_options['eddpdfi_url'] ) && $edd_options['eddpdfi_url'] ) {
		$eddpdfi_pdf->Rect( 203, 76.5, 0.5, $line_height_totals + 6, 'F' );
	} else {
		$eddpdfi_pdf->Rect( 203, 76.5, 0.5, $line_height_totals, 'F' );
	}

	$eddpdfi_pdf->SetFont( 'droidserif', '', 9 );
...
$eddpdfi_pdf->Cell( 0, 6, $country, 0, 2, 'L', false );
	...
	if ( $eddpdfi_pdf_downloads ) {
		...
		foreach ( $eddpdfi_pdf_downloads as $key => $cart_item ) {
			...
			if ( eddpdfi_item_quantities_enabled() ) {
				$eddpdfi_pdf->MultiCell( 102, $linecount * 5, $eddpdfi_download_title, 0, 'L', false, 0, 35 );
				$eddpdfi_pdf->Cell( 20, $linecount * 5, $cart_item['quantity'], 0, 0, 'C', false );
				$eddpdfi_pdf->Cell( 45, $linecount * 5, $eddpdfi_download_price, 0, 2, 'R', false );
			} else {
				$eddpdfi_pdf->MultiCell( 122, $linecount * 5, $eddpdfi_download_title, 0, 'L', false, 0, 35 );
				$eddpdfi_pdf->Cell( 45, $linecount * 5, $eddpdfi_download_price, 0, 2, 'R', false );
			}
		}
		...
		$eddpdfi_pdf->Cell( 0, 11, __( 'Total Paid', 'eddpdfi' )     . ' - ' . $total, 0, 2, 'L', false );

		$eddpdfi_pdf->Ln( 10 );

		if ( isset ( $edd_options['eddpdfi_additional_notes'] ) && ! empty( $edd_options['eddpdfi_additional_notes'] ) ) {
			$eddpdfi_pdf->SetX( 35 );
			$eddpdfi_pdf->SetFont( 'droidserif', '', 13 );
			$eddpdfi_pdf->SetTextColor( 149, 210, 236 );
...
add_action( 'eddpdfi_pdf_template_blue_stripe', 'eddpdfi_pdf_template_blue_stripe', 10, 10 );