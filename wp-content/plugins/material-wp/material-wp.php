...
class MaterialWP extends ParadoxFramework {
  ...
  public function enqueueAdminScripts() {
    ...
    // Enqueue base script
    wp_enqueue_script($this->id, $this->url('assets/js/scripts.min.js'), array('jquery', 'common'), false, false);
  ...
  public function checkBlacklist() {
    ...
    foreach($blacklist as $blacklist_page) {
      ...
      if ($blacklist_page && stristr($url, $blacklist_page)) {
  ...
  public function adminDynamicCSS() {
    ...
    if (!$this->isLoginPage() && !is_user_logged_in()) return;
